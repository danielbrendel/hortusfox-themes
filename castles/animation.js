document.addEventListener('DOMContentLoaded', function() {
    ...
    banner.classList.add('castles-crossfade');

    function imageAsset(num) {
        return window.location.origin + '/themes/castles/banner' + num + '.jpg';
    }
    ...
    function swapBanner() {
        ...
        banner.classList.add('castle-fade');

        setTimeout(() => {
            banner.style.backgroundImage = `url('` + imageAsset(currentIndex) + `')`;
            banner.style.setProperty('--current-bg', `url('` + imageAsset(currentIndex) + `')`);
            banner.classList.remove('castle-fade');
